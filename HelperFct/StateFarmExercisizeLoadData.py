########################################################################################################################
# Code to solve state farms interview- Question.-Clean and prepare your data:
#
# Step 0: Header
#      1: Helper fcts
#      2: main variable processing
#      3: Data load helper fcts. not really used tho
#      4: create sets by applying fcts.
#
# This code half follows usual SOP: with load functions, but since it was just me, I ended up just writting intermediate
#   files to .csv, as its quicker, even if it is hackier
#
# Remarks: missingness is not realistic-missing rates are low- adding all missing flags has a max of 1.
#          you won't see most value_counts/describes for post transformation as they got deleted.
########################################################################################################################
##### Step 0: Header #####
...
###### Step 3: Data load helper fcts. not really used tho #####
...
def LoadHoldout():
    Holdout = pd.read_csv('C:/Users/szreb/Documents/CodeSandBox/StateFarmExercisize/Data/exercise_05_test.csv',
                            sep=',')
    Holdout = ProcessDataFrame(Holdout)
    return Holdout

##### Step 4: create sets by applying fcts. #####

if __name__ == '__main__':
    TrainFull = LoadTraining()
    Train, Test = train_test_split(TrainFull, test_size=0.25, random_state= 31415)
    Train1, Train2 = train_test_split(TrainTemp, test_size=0.33333, random_state= 31415)

Holdout = LoadHoldout()

TrainFull = LoadTraining()
# Thought about just importing this module but the following is simpler for short projects.

TrainFull.to_csv('C:/Users/szreb/Documents/CodeSandBox/StateFarmExercisize/Data/TrainFullProcessed.csv',
              sep = '|',
              index=True)

Train1.to_csv('C:/Users/szreb/Documents/CodeSandBox/StateFarmExercisize/Data/Train1.csv',
              sep = '|',
              index=True)

Train2.to_csv('C:/Users/szreb/Documents/CodeSandBox/StateFarmExercisize/Data/Train2.csv',
              sep = '|',
              index=True)

Test.to_csv('C:/Users/szreb/Documents/CodeSandBox/StateFarmExercisize/Data/Test.csv',
              sep = '|',
              index=True)


Holdout.to_csv('C:/Users/szreb/Documents/CodeSandBox/StateFarmExercisize/Data/HoldOut.csv',
              sep = '|',
              index=True)
